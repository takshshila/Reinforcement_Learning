...
def policy_evaluation(P, policy, value_function, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for s,a in enumerate(policy):
			...
			new_value_function[s]=reward+gamma*prob*value_function[next_s]
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for s in range(nS):
			...
			for a in range(nA):
				prob, next_s, reward, terminal=P[s][a][0]
				action_reward.append(reward+gamma*prob*new_value_function[next_s])

			new_value_function[s]=np.max(action_reward)
		value_change=np.sum(np.abs(new_value_function-value_function))
